...
async function setup() {
    ...
    const vs = await fetch('./2.2_VertexShader.glsl').then(res => res.text())
    const fs = await fetch('./2.1_FragmentShader.glsl').then(res => res.text())
    window.program = compile(vs, fs)
    tick(0) // <-ensure only called at end of setup
}